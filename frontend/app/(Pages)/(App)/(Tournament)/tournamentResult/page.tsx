...
export default function TournamentResult() {
  ...
  const handleRaceResultRecordsInputChangebyIndex = (
    ...
    setRaceResultRecords((prevFormData) => {
      ...
      if (newFormData[index]) {
        // newFormData[index][name]  = (value as string);
      }
      return newFormData;
    });
  };
  ...
  const handleRaceResultRecordsCrewPlayerChangebyIndex = (
    ...
    setRaceResultRecords((prevFormData) => {
      const newFormData = [...prevFormData];
      (newFormData[index].crewPlayer[crewIndex][name] as any) = value;
      return newFormData;
    });
  };
  ...
  const validateRaceResultRecords = () => {
    ...
    const isError7 = raceResultRecords.some((record, i) => {
      ...
      return raceResultRecords.some((record2, j) => {
        ...
        return i !== j && record.org_id === record2.org_id && record.crew_name === record2.crew_name;
      });
    });
    if (isError7) {
    ...
    const isError4 = raceResultRecords.some((record, i) => {
      ...
      if (
        !record.laptime_500m &&
        !record.stroke_rat_1000m &&
        !record.stroke_rat_1500m &&
        !record.stroke_rat_2000m &&
        !record.stroke_rat_2000m &&
        !record.final_time
      ) {
        indexList3.push(i);
    ...
    const lapTime3 = raceResultRecords.some((record, i) => {
      if (
        record.stroke_rat_1000m &&
        !/^\d{1,2}:\d{2}\.\d{2}$/.test(record?.stroke_rat_1000m.toString())
      ) {
        indexList5.push(i);
      }
      return (
    ...
    const lapTime4 = raceResultRecords.some((record, i) => {
      ...
      if (
        record.stroke_rat_1500m &&
        !/^\d{1,2}:\d{2}\.\d{2}$/.test(record?.stroke_rat_1500m.toString())
      ) {
        indexList6.push(i);
      }
      return (
    ...
    const lapTime5 = raceResultRecords.some((record, i) => {
      ...
      if (
        record.stroke_rat_2000m &&
        !/^\d{1,2}:\d{2}\.\d{2}$/.test(record?.stroke_rat_2000m.toString())
      ) {
        indexList7.push(i);
      }
      return (
    ...
    const strokeRate = raceResultRecords.some((record, i) => {
      ...
      if (
        ...
        !/^\d{1,2}$/.test(record?.stroke_rat_500m.toString())
      ) {
        indexList9.push(i);
      }
      return (
    ...
    const strokeRate2 = raceResultRecords.some((record, i) => {
      ...
      if (
        record.stroke_rat_1000m &&
        !/^\d{1,2}$/.test(record?.stroke_rat_1000m.toString())
      ) {
        indexList10.push(i);
      }
      return (
    ...
    const strokeRate3 = raceResultRecords.some((record, i) => {
      ...
      if (
        record.stroke_rat_1500m &&
        !/^\d{1,2}$/.test(record?.stroke_rat_1500m.toString())
      ) {
        indexList11.push(i);
      }
      return (
    ...
    const strokeRate4 = raceResultRecords.some((record, i) => {
      ...
      if (
        record.stroke_rat_2000m &&
        !/^\d{1,2}$/.test(record?.stroke_rat_2000m.toString())
      ) {
        indexList12.push(i);
      }
      return (
          ...
          raceResultRecords.map((record) => {
            if (record.crewPlayer?.length < response2) {
              record.crewPlayer = record.crewPlayer.concat(
                // Array.from({ length: ((response2 - record.crewPlayer.length) as number) }, () => ({
                //   id: '', // 選手ID
                //   playerPhoto: '', // 選手画像
                //   playerName: '', // 選手名
                //   jaraPlayerId: '', // JARA選手コード
                //   playerId: '', // 選手ID
                //   sexId: '',
                //   height: null, // 身長
                //   weight: null, // 体重
                //   sheetName: '', // シート番号
                //   sheetNameId: null, // シート番号
                //   fiveHundredmHeartRate: null, // 500m
                //   tenHundredmHeartRate: null, // 1000m
                //   fifteenHundredmHeartRate: null, // 1500m
                //   twentyHundredmHeartRate: null, // 2000m
                //   heartRateAvg: null, // 平均
                //   attendance: '', // 立ち合い有無
                //   deleteFlg: null, // 削除フラグ
                //   addonLineFlg: null, // 追加行フラグ
                //   errorText: '', // エラーテキスト
                // })),
              );
            }
        ...
        <div className='flex flex-col gap-[20px] border border-solid p-[20px]' key={index}>
          ...
          <div className='flex flex-row justify-between'>
            {mode === 'update' && (
              <div
                onClick={() => {
                  handleRaceResultRecordsInputChangebyIndex(index, 'deleteFlg', (!item.deleteFlg as any as string));
                }}
